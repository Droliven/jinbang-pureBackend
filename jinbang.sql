...
-- ----------------------------
BEGIN;
INSERT INTO `paper` VALUES (1, '英语翻译测试', 1, 12, 12);
COMMIT;
...
-- ----------------------------
DROP TRIGGER IF EXISTS `trig_insert_paper_sum`;
delimiter ;;
CREATE TRIGGER `trig_insert_paper_sum` AFTER INSERT ON `buildpaper` FOR EACH ROW begin
declare temp int;
select sumscore into temp from paper where pid = new.pid;
if (temp = -1) then
    update paper set sumscore = new.score where paper.pid= new.pid;
    update paper set sumtimemin = new.timemin where paper.pid= new.pid;
else
